...
# -*- coding: utf-8 -*-
"""
Sistema Hikvision DS-K1T344MBFWX-E1 - VERSIÓN CORREGIDA
Autenticación: Credenciales embebidas en URL (MÉTODO CONFIRMADO FUNCIONANDO)
URL de prueba confirmada: http://admin:Oem2017*@192.168.0.222/ISAPI/System/deviceInfo
"""
...
class HikvisionDS_K1T344_Integration:
    ...
    Usa credenciales embebidas en URL (confirmado funcionando en navegador)
    """
    ...
    def __init__(self, ip, username, password, db_connection_string):
        ...
        # URL base con credenciales embebidas (MÉTODO CONFIRMADO EN NAVEGADOR)
        # NO codificar el asterisco en la contraseña - debe permanecer como *
        username_encoded = urllib.parse.quote(username, safe='')
        password_encoded = urllib.parse.quote(password, safe='*')  # Permitir asterisco
        self.base_url = f"http://{username_encoded}:{password_encoded}@{ip}"
        
        self.logger.info(f"Inicializado para DS-K1T344MBFWX-E1 en {ip}")
        self.logger.info(f"Usando credenciales embebidas en URL (método confirmado en navegador)")

    def crear_url_con_credenciales(self, endpoint):
        """Crear URL con credenciales embebidas - MÉTODO CONFIRMADO FUNCIONANDO"""
    ...
    def verificar_conectividad(self):
        """Verificar conectividad usando credenciales embebidas en URL"""
        try:
            # Usar exactamente la misma URL que funciona en el navegador
            url = self.crear_url_con_credenciales("/ISAPI/System/deviceInfo")
            
            self.logger.info("Verificando conectividad con credenciales embebidas en URL...")
            self.logger.info(f"URL de prueba: {self.base_url}/ISAPI/System/deviceInfo")
            
            # Usar requests básico sin autenticación adicional (las credenciales van en la URL)
    ...
    def obtener_contador_usuarios(self):
        ...
        try:
            url = self.crear_url_con_credenciales("/ISAPI/AccessControl/UserInfo/Count")
            response = requests.get(url, timeout=15, verify=False)
    ...
    def verificar_capacidades_dispositivo(self):
        ...
        for endpoint, capability in tests:
            try:
                url = self.crear_url_con_credenciales(endpoint)
    ...
    def crear_usuario_terminal(self, persona_id, nombre, apellido, activo=True):
        ...
        try:
            url = self.crear_url_con_credenciales("/ISAPI/AccessControl/UserInfo/Record?format=json")
            
            # Datos del usuario optimizados para DS-K1T344MBFWX-E1
            user_data = {
                "UserInfo": {
                    "employeeNo": str(persona_id),
                    "name": f"{nombre} {apellido}".strip()[:32],  # Límite de caracteres
                    "userType": "normal",
                    "Valid": {
                        "enable": activo,
            ...
            response = requests.post(url, json=user_data, headers=headers, timeout=30, verify=False)
            
            if response.status_code in [200, 201]:
                self.logger.info(f"Usuario creado exitosamente: {persona_id} - {nombre} {apellido}")
                return True
    ...
    def eliminar_usuario_terminal(self, persona_id):
        ...
        try:
            url = self.crear_url_con_credenciales("/ISAPI/AccessControl/UserInfo/Delete?format=json")
            ...
            response = requests.put(url, json=delete_data, headers=headers, timeout=30, verify=False)
    ...
    def enrolar_rostro_terminal(self, persona_id, facial_id, imagen_data):
        ...
        try:
            ...
            metadata_url = self.crear_url_con_credenciales("/ISAPI/AccessControl/UserInfo/SetFace?format=json")
            ...
            response = requests.post(metadata_url, json=face_metadata, headers=headers_json, timeout=30, verify=False)
            ...
            image_url = self.crear_url_con_credenciales("/ISAPI/AccessControl/UserInfo/SetFace?format=json")
            ...
            response = requests.put(image_url, data=imagen_data, headers=headers_binary, timeout=30, verify=False)
...
def main():
    ...
    print("SISTEMA HIKVISION DS-K1T344MBFWX-E1")
    print("="*50)
    print("Método: Credenciales embebidas en URL (confirmado en navegador)")
    
    # Configuración usando el método que funciona en el navegador
    ...
    print(f"URL de prueba: http://admin:***@{IP_DISPOSITIVO}/ISAPI/System/deviceInfo")
